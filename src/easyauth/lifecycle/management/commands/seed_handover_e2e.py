...
def _e2e_env(name: str, default: str) -> str:
...
class Command(BaseCommand):
    ...
    def handle(self, *_args: str, **_options: object) -> None:
        ...
        with transaction.atomic():
            message = self._seed_e2e_scenario()
        self.stdout.write(self.style.SUCCESS(message))

    def _ensure_e2e_people(self) -> tuple[UserMirror, UserMirror, UserMirror]:
        manager_username = _e2e_env("EASYAUTH_E2E_MANAGER_USER", DEFAULT_MANAGER_USER)
        subject_username = _e2e_env("EASYAUTH_E2E_SUBJECT_USER", DEFAULT_SUBJECT_USER)
        peer_username = _e2e_env("EASYAUTH_E2E_PEER_USER", DEFAULT_PEER_USER)
        ...
        return manager, subject, peer

    def _seed_e2e_scenario(self) -> str:
        downstream_port = _e2e_env("EASYAUTH_E2E_DOWNSTREAM_PORT", DEFAULT_DOWNSTREAM_PORT)
        secret = _e2e_env("EASYAUTH_E2E_DOWNSTREAM_SECRET", DEFAULT_SECRET)
        handover_url = f"http://127.0.0.1:{downstream_port}/api/v1/easyauth/lifecycle/handover"
        manager, subject, peer = self._ensure_e2e_people()

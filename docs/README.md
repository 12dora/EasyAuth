...
- [质量门禁](operations/quality-gates.md) —— CI 作业与静态检查边界
...
- [ADR-002：`MANAGED_USERS` 管理范围契约](decisions/ADR-002-MANAGED_USERS管理范围契约.md)
- [ADR-003：特权入口与产品能力边界](decisions/ADR-003-特权入口与产品能力边界.md)
- [ADR-004：旧兼容形态清理](decisions/ADR-004-旧兼容形态清理.md)

## 待实施计划

- [NetBird 服务端改造方案](plans/2026-07-07-netbird-server-fork-plan.md) —— 只涉及外部 NetBird
  management fork，本仓库无法证明已实施，保留为待执行设计。
...
7. **`plans/` 只放尚未实施、有明确负责范围的计划。** 实施完成后把长期有效的结论合并进
   architecture / api / ADR，然后删除计划文件。
8. **写给人看。** 优先讲清"是什么、为什么、什么时候会踩坑"，不堆砌实现细节和内部编号。

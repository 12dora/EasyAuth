...
岗位模板是入职授权和转岗差异的基准事实。模板本体只保存当前名称、说明、启停状态和当前修订指针;每次新建或完整编辑模板都会创建新的 `OnboardingTemplateRevision` 与不可变 `OnboardingTemplateRevisionItem` 集合。编辑不得删除或重写旧修订项。

转岗差异生成时,`TransferPlan` 同时绑定模板本体和精确模板修订,并把差异条目的 `key`、应用、目标类型、目标名称、范围和期限配置写入冻结 `grant_diff`。确认转岗差异时以冻结 `grant_diff` 作为新增授权的唯一执行事实;`TransferPlan.new_template_revision` 只用于标识该方案来自哪一版模板,不得重读模板当前修订或绑定修订项推断执行内容。模板后续编辑产生的新修订不会自动使旧计划失效;只有交接单不再开放、授权快照版本不匹配、方案版本不匹配或确认载荷不属于冻结差异时,确认接口才返回冲突或校验错误。

岗位模板不提供硬删除能力。已创建模板如不再用于新入职或新转岗,必须通过启停状态停用;旧 `DELETE` 接口返回稳定冲突,不会级联删除不可变修订。

...
| 紧急撤权时当前授权含组织授权来源成员 | `409`，不撤权、不记成功审计；检查与撤权同锁同事务 |
| 直接授权展开**当前**含 `MANAGED_USERS` 的响应行时组织目录不可用 | `503`，授权写入与成功审计回滚。目录 HTTP 发生在写事务加锁之前，锁内只读已预热的缓存 |
| 直接授权空提交收回后展开**历史**行时组织目录不可用 | 收回成功；历史行不解析管理对象、不访问目录 |

...
- 审批人动态取申请人的直属上级，并由服务端按 Authentik 或 DingTalk 身份映射验证。
- 最终审批人集合必须严格等于该 active 直属主管，不允许手动改填 App owner 或其他用户。
- 如果找不到 active 直属主管，申请必须快速失败；应先修复权威目录中的主管关系再提交。
- 保留整体申请和审批人关系记录，不新增客户端可控的审批人来源字段。
- 申请类型保留现有 `grant`。
...
- 自助申请测试覆盖直属主管解析、手动改填被拒绝、找不到上级和审批人 inactive。

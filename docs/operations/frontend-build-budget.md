...
- 全部 JavaScript 原始体积不超过 `2460 KiB`（Ant Design 表格地基落地后上调到 1700，2026-09-08 控制台
  ...
  2026-09-21 状态健康「用量监控」引入 recharts（含 d3，约 404 KiB）并拆为只被趋势图懒加载的独立 `recharts`
  chunk，加上 `UsageMonitorTab` 异步路由后实测 2419.8 KiB，同步 chunk 额度不变，总量上调到 2460；

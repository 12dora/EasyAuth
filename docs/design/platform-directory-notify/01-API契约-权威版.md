...
- legacy 引用只在全部目录作用域中唯一匹配时解析；多作用域匹配返回
  ...
  `candidate_refs`），畸形 scoped ref 返回 `422 VALIDATION_ERROR`
  （`details.reason=invalid_directory_ref`）。通知 POST 则把 legacy 用户歧义记录为
  逐收件人 `failed(USER_AMBIGUOUS)`，不以 HTTP 409 拒绝整条消息。
...
| `VALIDATION_ERROR` | 422 | 参数缺失/格式错/超长（含畸形 scoped ref、msg 2048 字节超限） |
...
- `recipients` 元素应为目录返回的 opaque `user_ref`，**1~500 个**；

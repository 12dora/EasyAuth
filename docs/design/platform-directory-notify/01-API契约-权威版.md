...
- 额外前置条件（本次新增）：应用必须由超管开通对应平台能力
  （`directory` / `notify`，见第 5 篇 §1）。未开通 → `403 PERMISSION_DENIED`，
  message 明示「应用未开通目录能力。」/「应用未开通通知能力。」
...
| `PERMISSION_DENIED` | 403 | app_key 不匹配 / 应用禁用 / 能力未开通 |
| `VALIDATION_ERROR` | 422 | 参数缺失/格式错/超长（含 msg 2048 字节超限） |
| `NOT_FOUND` | 404 | 用户/消息不存在或不属于本 app |
| `CONFLICT` | 409 | dedup_key 已存在但载荷不同 |
| `THROTTLED` | 429（带 `Retry-After` 头） | 速率/每日配额超限 |
| `DEPENDENCY_UNAVAILABLE` | 503 | （预留，目录端点当前纯本地镜像查询，不出现） |
...
**数据新鲜度承诺：滞后上游最多一个同步周期（300s）**；所有目录端点返回
`Cache-Control: private, max-age=60`，消费方可安全做 60s 本地缓存。

字段最小化：**不返回** email、手机号（未入镜像）、工号、unionId、corp_id、
完整主管链（裁决理由见第 5 篇 §2）。
...
- 引用不存在 → `404 NOT_FOUND`。
- 边界：若用户曾登录（有 `user_id`）但已从钉钉目录移除，返回
  `active: false`、`departments: []`、`manager: null`（详情仍可查，供下游做
  历史数据展示）。
...
24h 窗口）；对账属尽力而为，`sent` 是消费方可依赖的最低保证。事件型通知
...
| N-5 | 收件人状态 `pending/sent/failed/throttled`，字段 `error?` | 增加 `delivered`；`error` 拆为机器可读 `error_code` + 人读 `error`；补七值枚举 | 钉钉「受理≠送达」，静默流控只能靠回执对账发现；下游需要可编程的失败分类 |

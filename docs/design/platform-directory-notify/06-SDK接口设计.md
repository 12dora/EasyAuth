...
def send_notification(
    ...
    *,
    ...
    recipients 元素为目录条目返回的 opaque user_ref; template 取
...
兼容；目录歧义为 409、畸形 scoped ref 为 422，通知歧义则成为逐收件人
`USER_AMBIGUOUS`。
...
- 错误处理矩阵：`404` 按 `details.reason` 分支；`409` 还包括 legacy ref 歧义，
  `422` 还包括畸形 scoped ref，两者均修正请求、不原样重试；
  `429` 按 Retry-After 重试；`401` / `403` 视为凭据或 capability 配置问题；
  `5xx` 与网络错误标记 `retryable=true`。
- 建议权限查询、directory、notify 各用一条 credential；平台能力必须同时通过
  App 层与 credential 层，manifest 声明不会自动开通任一层。

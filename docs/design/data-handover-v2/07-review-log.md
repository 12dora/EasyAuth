...
### 第五轮（2026-08-10，7 路并行，Claude sub agent）—— 又挖出 6 个 BLOCKER，**收敛但未干净**

> **换了复核工具**：前四轮用 codex（gpt-5.6-sol），本轮 codex 额度耗尽（要到 8/16 恢复），
> 改用 Claude sub agent 七路并行。切片与前几轮相同（契约 / `01` 数据层 / `01` 接口与 SDK /
> `02` 前端 / `03` / `05`+`08`+`09` / 端到端与跨文档）。

共 **62 条**，其中 **BLOCKER 6**、MAJOR 24、MINOR 32。**采纳 61 条，驳回 1 条**
（agent 称 `RecurringTemplateRow.assigner_dingtalk_user_id` 可空，实际
`recurrence.py:90-98` 是 `nullable=False`，文档原文正确 —— 又一次印证 §4 的方法学备忘）。

**这一轮的性质与前几轮不同**：找出的不再是"设计没想到"，而是**前几轮的修法自己带进来的新缺陷**，
以及**文档声称的代码事实不成立**。六个 BLOCKER：

| # | 问题 | 后果 | 落点 |
|---|---|---|---|
| 1 | **`async_attention_required` 是个没有出口的终点（第 5 个死锁）** | 第三轮为了修「轮询耗尽释放租约」引入了这个状态，但轮询任务只扫 `async_pending`，状态一改就再没人扫它；枚举里也从没登记过它，前端 TS 联合类型拿到它直接落 `never` 分支。此时 retry（状态不是 `failed`）/ skip / cancel（租约未释放）全被拒，无任何 beat 推进，而租约还占着 `(subject, app)` —— 该员工在该 APP 上**再也建不出可执行的单** | `00` §6.2/§10.6、`01` §2.2/§6.3/§7、`02` |
| 2 | **413 分批第 2 批起必然卡死** | 每批强制重新 preview，第 1 批搬完的 `asset_id` 不再属于当事人、其 override 被强制删除，重算 `assignment_hash` 必然不等 → 第 2 批被拒；而 `completed_batches>0` 时三个改分配端点全 409，**没有重建计划的入口**。第三轮的第 3 个死锁换了个触发者回来了（不是用户改分配，是系统自己在强制的 re-preview 里改的） | `01` §2.4.1.1 |
| 3 | **`retry` 没写怎么拿租约，两个读法都是坏的** | 照字面「只追加 delivery 就发网」→ 不持租约，违反冻结的互斥，两个 execute 同时在途；严格遵守「claim 失败不许发网」→ 没有 active 租约可 claim，**retry 永远发不出去**，§10.6 里标注可重试的 400/5xx 全部落空 | `01` §2.4.1 |
| 4 | **传输层失败的投递记录违反库层 CHECK，写不进去** | 连接失败/超时/SSRF 拒绝/响应体非 JSON 抛的 `HookCallError` 都不带 `status_code`，而 CHECK 是 `outcome IN (sent,superseded) OR http_status IS NOT NULL`。下游宕机（最常见故障，也是租约存在的全部理由）时写 `failed` 会抛 IntegrityError，整个「写 failed + 释放租约」事务回滚 → 这张单既不能执行也不能跳过取消 | `01` §2.4.1/§2.4.2 |
| 5 | **冻结的小写错误码在本仓库无处安放** | `ErrorCode` 是 `@unique` 的 9 个大写值，后端只能返回 `CONFLICT`，而 `02` 按 `snapshot_stale` / `overrides_version_stale` / `out_of_managed_scope` 分支 → **永远不命中**，412 重新预演、409 自动重载、403 与 503 的文案区分全部退化成通用报错，**且没有任何测试会红** | `01` §6.1、`02` |
| 6 | **`05` 漏抄了 `08` 冻结的第 4 条 revision 与第 2 个 nullable 列** | 「接收人原先不是项目成员 / 不是协作人」是最常见的交接场景，INSERT 直接 `NotNullViolation`，整批回滚成 5xx；merge revision 还会少一个 parent，`upgrade head` 留下双 head | `05` §4.7 |

**另有两条 BLOCKER 级的连锁问题**（归在 MAJOR 里，但性质相同）：
控制台缺资产/明细/覆盖四条端点（`02` 的控制台向导直接内嵌门户组件，实现者只能不做或撞禁令）；
`PROJECT_LOCKED` 一刀切转 423 会造出「退回 pending → 重新预演 → 又 423」的死循环
（`_WRITE_LOCKED_STATUSES` 把项目终态也算写锁，而终态的锁没有任何人能解除）。

**本轮新增的冻结口径**（下游必须照抄，不得各写各的）：

1. **412 与 409 的校验顺序是规范的一部分** —— 必须先重算全量摘要、后逐条；
   摘要不一致一律 412 且零写入；**归属在 preview 之后变化的一律 412，不得用 409 表达**。
   不写死顺序的话，逐条校验天然写在行循环里、比重算摘要更早，一次普通竞态就被判成
   不可重试的 `failed`，而且两个下游会给出不同结局。
2. **错误细码落 `details.reason`，`error.code` 保持粗分类**（本仓 `ErrorCode` 是全 API 共用的）。
3. **租约释放表按「结局」封闭，不是按「状态码」** —— 无状态码的传输失败也必须有一行。
4. **`task_id` 校验用 `re.fullmatch()`，不能用 `re.match(r"^...$")`** ——
   Python 的 `$` 放行结尾换行，`"137:1\n"` 会成为第二个幂等键。
5. **`page_size` 的钳制只发生在 EasyAuth 门户边界**（钳制后的值才转发），
   下游 webhook 边界三项一律 422。两边规则不同是有意的。

**四个历史死锁的复核结论**：#4（非末批提前置 `done`）**已真正修好**；
#2（同步终态释放租约）**基本修好但表不穷尽**（漏 400、429、传输失败、授权转移失败四条，已补）；
#1、#3 **修法本身引入了新死锁**（即上表 1 与 2）。

抽查 `07` 里五条「已修复」条目，**五条都确已落到对应文档**；
抽查下游有无擅自放宽冻结契约，**六项逐字一致**。

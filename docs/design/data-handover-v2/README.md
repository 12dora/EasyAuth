...
> **当前状态：可以开工。A5 的解阻材料已备齐，三道门禁待批。**
>
> - **代管授权整体砍掉**。悬置期的可见性改由交接单自身的资产明细承担（契约 §7）。
>   随之取消：两份下游前端改造、EasyTrade 的 B3、EasyProject 的 P1、ADR-002 §19 修订
>   —— 离职者不再进入任何人的 `MANAGED_USERS`，下游列表按 owner 过滤时根本查不到这批数据，
>   F1/F2/F3 三个故障都不会发生。
...
>
> **文档里凡是写「早期版本这样写会怎样」的地方，都不是编辑史，是陷阱警告** ——
> 那些写法看着都很自然，但照做会当场炸或静默出错，原因就写在旁边。别跳过。
>
> 实现时如果发现某处照做不通，**先怀疑文档**，把问题提出来，不要自己兜底 ——
> 静默兜底是这套设计反复要消灭的东西。
...
> **这段边界在 `00` §14 与 `05` §5.2/§7 里也各出现过一次。以本表为唯一口径**，其余两处只做引用。

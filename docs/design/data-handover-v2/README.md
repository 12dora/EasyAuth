...
> **当前状态：五轮对抗式复核完成，A5 的解阻材料已备齐（三道门禁待批）。**
>
...
>   **材料齐备 ≠ 已解锁**：CCR 目前是 `PROPOSED`，裁定正文也还没合进
>   `EasyProject/contracts/ownership.md`。三样凭据集齐前，A5 只能做「现在就能做」的三项。
>
> 五轮复核累计约 380 条发现，全部逐条对照代码验证后落文档。
> 第四轮加了一片**端到端走查**（跟着一张单从建单走到完成/失败/升级/分批/异步/取消），
> 修掉了**四种「走不出去的状态」**。
>
> **第五轮（换用 Claude sub agent，codex 额度到 8/16 才恢复）又挖出 6 个 BLOCKER**，
> 性质与前几轮不同：找出的不再是「设计没想到」，而是**前几轮的修法自己带进来的新缺陷**
> （第 5 个死锁、413 第 2 批必然卡死、`retry` 没写怎么拿租约），
> 以及**文档声称的代码事实不成立**（冻结的小写错误码在本仓无处安放、
> 传输失败的记录违反库层 CHECK 根本写不进去）。
> 意思很直接：**这套文档已经足够能开工，但它不是"复核干净了"的状态** ——
> 实现时如果发现某处照做不通，先怀疑文档，去 `07` 看有没有同类记录，别自己兜底。
> **开工前必读 [`07-review-log.md`](07-review-log.md)** —— 那里列了历轮找出的
> 「照做必错」问题，都是实现时最容易踩的。
...
|---|---|
| A1 EasyAuth 后端 | 立即 |
| A2 EasyAuth 前端 | 立即 —— `01` §6 已是冻结文本，**不需要等 A1"提交"** |
...
**只有一件事必须排队：SDK 0.4.0。其余全部并行。**

| 波次 | 谁 | 做什么 | 前置 |
|---|---|---|---|
| **第 0 天** | 你 / AG-00 | 提交 [`09`](09-easyproject-ccr.md) 的 CCR、[`08`](08-easyproject-ag00-rulings.md) 的两份裁定 | — |
| | A1a | **只做 SDK 0.4.0**（见上方交付清单与两提交发布流程） | — |
| | A1 | 提交 ADR-002 §36 修订（1 个工作日内出结果） | — |
| | A2 | 直接开工 —— `01` §6 已是冻结文本，**不需要等 A1 "提交"** | — |
| | A3 | **先单独做锁序整改 PR**（`03` §6 第 0.5 步），与 SDK 无关 | — |
| | A5 | `05` §7 第 0.5 与第 2 步（修 `quality-gate.sh`、身份映射、`hint`、终态谓词） | — |
| | A1b | `01` §11.1 的 §2 模型 + §5 执行链 | **无** —— SDK 与本仓库实现互不依赖（`01` §11.1 明写「A1a 与它们完全独立，可以第一天就并行」）。A1 是全项目最大的一片，压在一个纯打包工作后面是白等 |
| **SDK 发布后** | A1c | `01` §11.1 的其余分工 | A1b 的模型层落地 |
| | A3 | `03` §6 第 2 步起 | 本仓库 `VENDORED.md` 更新完 |
| **三凭据齐后** | EasyProject 六条线 | `08` §1.6 第 3 步，**六条互不依赖，完全并行** | ownership 合入 SHA + CCR APPROVED + SDK |
| | A5 | M06 编排（`05` §7 第 4 步起） | 逐个收 owner 的 handoff |

**第一个跨仓检查点安排在 SDK 发布之后立刻做**，不要等到最后：
`00` §14 的联调门禁（`webhook.test` 对每个 APP 返回 200 且 descriptor 解析正常）。
这一条会同时打到**发送端两个出口注入 `event_type`、SDK 在短路前校验、下游改接 SDK 内核、
descriptor 三道白名单** —— 四件事错一件就过不去，早发现比晚发现便宜得多。

**各片的量级**（用可数的东西标，不给人天）：

| 片 | 量级 | 可数依据 |
|---|---|---|
| A1 | **最大** | 8 张新表、3 个 app 的迁移、32 个 HTTP 端点（14 门户 + 18 控制台）、3 个 beat 任务、13 个测试文件 |
| A3 | 中偏大 | 8 类资产、3 个事件、1 个迁移、9 个测试；**外加三条既有锁路径的整改（回归面大于交接本身）** |
| A2 | 中 | 2 个共享组件、4 个门户界面、4 处控制台改造、7 个测试文件 |
| A5 本身 | 小 | 只做编排与契约适配，一行业务 UPDATE 都不写 |
| EasyProject 六条线 | 合计中偏大 | 6 个 `system_handover` 命令 + 4 条迁移；其中 OpenProject 投影那条最容易整片漏掉 |
...
1. **EasyProject CCR**：给既有操作 `postEasyauthLifecycleHandover` 补 10 个错误码。

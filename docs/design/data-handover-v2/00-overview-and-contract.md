...
| D4 | 悬置期的可见性靠**交接单本身** | ~~发限时代管授权~~（已废弃，见 §7）。改为在交接单里直接展示各 APP 的资产明细，主管据此判断归属；**不发放任何临时授权、不新增 scope、不碰 `MANAGED_USERS`** |
...
（例：`"137:4"`）。长度上限 **64 字节**，且必须完全匹配 **`[0-9]+:[0-9]+`** ——
两段都不得为空，不得出现第三段。下游对不匹配的 `task_id` 返回 **422**。

> 只写「字符集限 `[0-9:]`」是不够的：`":"`、`"1:2:3"` 都能通过字符集校验，
> 而下游要把 `task_id` split 开来做日志反查（见下），畸形值要到落库那一步才暴露。
>
> **Python 侧必须用 `re.fullmatch()` 或 `\A...\Z`，不能用 `re.match(r"^...$")`。**
> `$` 在结尾换行之前也匹配，`"137:1\n"` 会被放行 —— 于是它和 `"137:1"` 成了两个不同的
> `task_id`，两个下游各自落两行回执、推进两次 generation 水位、执行两次归属改写，
> §10.5 的三元组幂等被整个绕过。两个下游的拒绝用例都要覆盖 `"137:1\n"` 与 `" 137:1"`。
...
> **原方案是「给主管发限时代管授权，让他在业务系统里看到离职者的数据」**，
> 该方案经验证不可实现且带安全漏洞（四条独立致命问题，其中最要命的一条见下），已**整体废弃**。
...
| 主管链已到顶 | `assignee_state = superuser_pool`，`assignee = NULL`。**不发个人通知** —— 超管资格只在请求期通过 Authentik 组交集判定（`admin_console/authz.py`），没有可枚举的本地超管名单。改为在超管控制台顶部常驻不可关闭的「N 张交接单待认领」告警条 |
...
| 把 departed 用户并入 `MANAGED_USERS` | **取消** | **这本身就是提权** —— 主管会因此获得对离职者全部数据的写权限，而交接的目的只是「看清楚有什么」 |
...
| EasyTrade / EasyProject 前端改造 | **取消** | 离职者不会出现在下游列表里，F1/F2/F3 三个故障都不会发生 |
...
- 控制台顶部常驻告警：「N 个 APP 未接入数据交接，M 张交接单被阻塞」。**不可关闭**（关掉就等于回到静默状态）。这是本期向超管示警的**唯一**形式 —— 无法枚举超管收件人，见 §7.4。
...
- 签名：`X-EasyAuth-Signature` = HMAC-SHA256(secret, `timestamp + "." + raw_body`)，
  ...
  > —— 都断言 422。
  >
  > **不要写"篡改 delivery 头必须返回 422"这条用例，它做不到。** delivery 既不在签名覆盖范围内，
  > body 里也没有对应字段，下游拿不到任何信息去判断它被改过。
  > 补偿是别的：**handover 的幂等键是 `(task_id, generation, batch_id)`，不是 delivery id**
  > —— 改了 delivery 也换不出一次重复执行，同一三元组仍按幂等重放处理。
  > 把 delivery 纳入签名另立 CCR。
  >
...
- `batch_id` 是 EasyAuth 生成的单调递增整数，**同一 generation 内可以有多批**。
  它在 EasyAuth 内部的字段名是 `batch_seq`（`HandoverExecutionBatch.batch_seq`、
...
2. **响应体分两处存，可见范围不同**（原先"原样存进 `last_error` 并展示给人看"已废弃）：
   ...
   | `action.last_error_raw`（仅超管控制台，且每次读取写审计） | 下游响应体的**脱敏投影**，UTF-8 截断 2000 字节。**不是原文**：入库前先跑一遍与 `last_error` 相同的脱敏（密钥/token 形态串、URL 凭据、人员标识、连接串），只是保留更多上下文字段 | 仅超管 |

   白名单只认 `code` / `message` / `traceId` 三个键（两个下游的错误体恰好都有，见上）；
   其余字段、非 JSON 体、异常堆栈**一律不进 `last_error`**，只进（脱敏后的）`last_error_raw`。

   > **限制可见范围不等于可以明文入库。** 业务表会进备份、会被导出、会被 DBA 看到；
   > 下游异常里可能带数据库连接串、token、邮箱、查询参数。
   > 因此 `last_error_raw` 存的是**脱敏投影**而非原文。
   > **投递账本（`HandoverDeliveryAttempt.response_payload` / `error_text`）同样**：
   > 只存 HTTP 状态、响应体的 SHA-256、以及白名单脱敏摘要，并冻结长度上限；
   > **不存未限长、未脱敏的副本**。
   > 确需原文排障时，走独立的加密短 TTL break-glass 存储，不写普通业务表。
...
D11 是冻结决策，**下游文档不得单方面豁免**。只有以下两条属于正式例外，
其余一律照 D11 执行：
...
| ~~超管池待认领~~ | — | — | **本期不做**：无法枚举超管收件人。改为控制台常驻告警条（§7.4）。建立权威超管收件人镜像后再恢复 |
| 数据已移交给你 | 接收人 | execute 成功 | 即时（D12：仅通知，无需同意） |
| 你的数据已被移交 | 转出方（在职时） | `reassign` execute 成功 | 即时 |
| 下属数据发生移交 | 发起人的上一级主管 | `reassign` execute 成功 | 即时 |
| ~~APP 未接入告警~~ | — | — | **本期不做**，同上。改为控制台常驻告警条（§9.2） |

---

## 14. 并行开工边界

> **本节只是摘要，权威口径在 `README.md` 的「开工总调度」。**
> 三个后端**不能**同时开工：A3/A5 要用 A1 产出的 SDK 与契约样本。
...
**三条 lane，覆盖面完全不同，不能互相顶替**：

| lane | 跑什么 | 允许 mock 吗 | 证明了什么 |
|---|---|---|---|
| **① repo-local 契约 lane** | fixture 用 ORM 直接造（subject / manager / 两个接收人 / App / task / action），下游用一个**签名确定的假服务**顶替 | 允许（假下游） | payload 形状、状态机、幂等、租约。**证明不了**真实路由、真实 HMAC、真实身份映射 |
| **② 跨仓真实服务 lane** | 三个仓库的真实服务 + 真实数据库同时起 | **禁止任何 mock** | 步骤 3、步骤 5「接收人在业务系统里真的看得到」 |
| **③ 外部系统 lane** | 真钉钉、真 Authentik | **禁止任何 mock** | 步骤 2 的离职标记与账号禁用 |

> **①全绿不等于通过。** 假下游会让真实路由、HMAC、身份映射、可见性、
> 以及 Authentik 禁号这些环节**一个都没被验证**，而它们恰恰是最容易出问题的部分。
> 步骤 3（主管在 EasyTrade 看不到离职者的客户）与「接收人真的看得到」必须归入 lane ②；
> 账号禁用归 lane ③（当前单测用的是 fake client）。
>
> **未配置外部系统时，lane ②/③ 的条目记 `NOT_RUN`，不得记 PASS。**
> 每条都要写明：启动命令、fixture、断言、检查人、证据形式。

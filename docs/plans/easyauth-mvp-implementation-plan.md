...
EasyAuth MVP 应先让试点内部应用可以接入授权查询，再补齐身份同步、员工访问申请、DingTalk 审批处理、撤权、过期清理和试点接入文档。当前仓库只有文档，因此首批任务需要先初始化 Django 工程和质量门槛，再实现领域模型与服务。
...
**说明：** 使用 Django SSR 和 HTMX 增加员工侧申请提交路径，并由 `AccessRequestService` 支撑。
...
- [ ] `approved` 与 `grant_applied` 保持分离。

**验证：**

- [ ] 单元测试覆盖申请校验和状态转换。
- [ ] 浏览器冒烟测试覆盖 app 选择、role 选择、生命周期校验和提交流程。
- [ ] `pytest tests/unit/access_requests tests/integration/portal`
...
- [ ] 重放回调保持幂等。
- [ ] `pytest`、`ruff check .`、`basedpyright` 和 browser smoke 通过。
...
- [ ] Expired grants 会转为 revoked、递增 version，并写入 `grant_expired`。
...
### 任务 14：发布试点接入包和端到端 smoke

**说明：** 输出试点接入文档，并建立端到端 smoke 路径，证明 MVP 可以通过预期使用界面工作。
...
- [ ] E2E smoke 覆盖 Authentik 登录、CRM role 申请、DingTalk mock 审批、grant application 和 CRM 权限查询。
...
- [ ] 员工申请、DingTalk 审批、grant application、revocation 和 expiry 行为都能通过测试和 smoke 流程观察。
...
- 核心模型存在后，任务 9 可以和任务 10 并行。
- 任务 7 稳定后即可开始任务 14 的文档工作，再随着 OAuth2、DingTalk、撤权和过期行为落地持续更新。

## 必须顺序执行的工作

- 任务 1 必须最先完成，因为其他任务都依赖项目结构和工具链。
- 任务 5 必须先于任何修改 grants 的流程，因为 `GrantService` 是唯一授权写边界。
- 任务 7 应先于门户重型工作，因为 MVP 成功标准依赖内部应用授权查询接入。
...
| revoke、expiry 和 approval 路径的 grant version 递增不一致 | 高 | 所有 grant 变更都经过 `GrantService`，并为每条变更路径测试 old/new version 和审计事件。 |

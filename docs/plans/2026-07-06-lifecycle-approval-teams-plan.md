...
1. 管理员手动发起；目录同步检出部门变更时在人员列表给"部门已变更"提示作为线索，但不自动建单——转岗是人事决策，系统不猜。
   - **更新**：该"部门已变更"提示已从前端移除（人员列表不再展示徽标）。后端 `department_changed_at` 仍由目录同步维护、转岗完成时清除，仅作内部线索、不再对外展示。
   - **修复（假转岗）**：`_primary_department_name` 原来取 `departments` 列表的第一项，上游顺序不稳定会让多部门员工的首选部门在两次同步间抖动，误置 `department_changed_at`。现改为按 `dept_id` 稳定排序后取首个部门，从源头消除假转岗。

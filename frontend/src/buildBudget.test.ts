...
describe("前端构建预算脚本", () => {
  test("校验 Vite manifest 中的入口、同步 chunk 与异步路由 chunk", () => {
    ...
    try {
      ...
      expect(output).toContain("异步路由 chunk: 19");
    } finally {
...
function requiredRoutes() {
  return [
    ...
    ["src/pages/console/systemHealth/SystemHealthPage.tsx", "SystemHealthPage"],
    ...
    ["src/pages/portal/PortalHandoverList.tsx", "PortalHandoverList"],
    ["src/pages/portal/PortalHandoverDetail.tsx", "PortalHandoverDetail"],
  ].map(([key, name]) => ({

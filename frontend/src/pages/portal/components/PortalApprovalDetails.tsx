...
/**
 * 审批列表「申请内容」列: 只放「我的权限 → 权限详情」同一套条数浮层。
 * 申请类型与权限组名不再进单元格; 组名在浮层按来源分组列出, 完整事实留在决定弹窗。
 */
export function approvalContentSummary(approval: PortalApprovalRow): ReactNode {
  return <GrantPermissionsCell row={approvalPermissionsRow(approval)} />;
}

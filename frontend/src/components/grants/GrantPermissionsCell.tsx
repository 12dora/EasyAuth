...
/** 展开权限的来源组; 门户与控制台的组行都带着更多字段, 这里只取用到的两个。 */
export interface GrantPermissionsGroup {
  key: string;
  name: string;
}

/** 展开后的单条权限; `source_key` 为空表示后端没有给出来源(仅直接授权会出现)。 */
export interface GrantPermissionsEntry {
  source_type: string;
...
export interface GrantPermissionsRow {
  groups: readonly GrantPermissionsGroup[];
  grants: readonly GrantPermissionsEntry[];
}
...
export function GrantPermissionsCell({ row }: { row: GrantPermissionsRow }) {
 ...
 * 方向键才滚得动最近的滚动祖先。
 */
function GrantPermissionList({ row, locale, t }: { row: GrantPermissionsRow; locale: Locale; t: Translator }) {
  return (
...
function buildSections(row: GrantPermissionsRow, locale: Locale, t: Translator): GrantPermissionSection[] {

...
6. **webhook 验签**: `verify_webhook` 校验 EasyAuth 反向推送(审批结果/交接/权限传播事件)的签名与时间戳;
...
8. **权限传播事件端点**: `easyauth_events_router` 接收 `grant.changed` /
   `catalog.changed` 异步推送, 供下游立即刷新用户权限快照或使目录缓存失效。
...
EasyAuth 在用户当前授权或应用权限目录变化后, 向 manifest `webhook.events_url`
(控制台字段同名 `events_url`) 异步 POST。下游应挂载:
...
def on_grant_changed(event: WebhookEvent) -> None:
    # 立即拉取 event.payload["user_id"] 的权限快照, 并核对 snapshot_version 已前进。
    ...

def on_catalog_changed(event: WebhookEvent) -> None:
    # 将该应用全部缓存快照标为过期, 下次权限检查/登录时再拉取。
    ...
...
app.include_router(
    easyauth_events_router(
        lambda: settings.easyauth_webhook_secret,
        EventCallbacks(
            on_grant_changed=on_grant_changed,
...
默认路径 `/api/v1/easyauth/events`。签名失败默认 **401**; `webhook.test` 与未知事件
的处理与生命周期端点相同(200 `{"ok": true}` / 422 `unsupported_event`)。
两个回调均可省略, 省略时仍确认收讫。

不使用 FastAPI 时调用纯函数内核 `events_http_response`。

...
- 顶层 `capabilities` 用于申明应用需要的 EasyAuth 平台能力。申明不等于开通，仍需
  EasyAuth 超管显式启用。校验时会按服务端语义对字符串做 trim/去重判断，并接受未来新增的
  capability；`validate_manifest` 仍返回传入的原对象，不会原地改写。
...
目录能力必须先由 EasyAuth 超管为应用显式开通。目录中的员工不一定已经完成 SSO，因此
`user_id` 可能为 `null`；此时应保存并使用 `dt:<dingtalk_user_id>` 引用。同步下游员工时，
应使用接口返回的目录 generation、快照时间与 stale 状态判断快照是否完整可信，不能仅凭单次
列表缺失就停用本地用户。

```python
users = client.search_directory_users(include_inactive=True, page=1, page_size=100)
...
通知能力也必须由 EasyAuth 超管显式开通。当前 capability 按整个 App 授权，同一 App 的已有
credential 会共享这一权限面。收件人必须由下游后端根据业务事件与授权规则计算，禁止提供把
前端任意 `userRef` 原样透传给 `send_notification` 的接口。

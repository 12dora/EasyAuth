...
def _patch_usage(
    ...
    *,
    ...
    monkeypatch.setattr(
        "easyauth.usage.alerts.last_60_minutes",
...
def _silent_send(monkeypatch: pytest.MonkeyPatch, result: str | None = None) -> list[object]:
    captured: list[object] = []

    def _fake(events: object, now: object, config: object) -> str | None:
...
def test_failed_event_is_retried(monkeypatch: pytest.MonkeyPatch) -> None:
    _silent_send(monkeypatch, result="发送方应用未就绪")
    _patch_usage(monkeypatch, day={"api": 2500})
